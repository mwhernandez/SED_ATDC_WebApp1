	...
	public List<Area> getPersonas(String nroCorrelativo, String areaDerivado, String nanoLlave) {
		...
		jdbcTemplate
				.query("select s.nderivado as codigo,UPPER(t.VAPEPATERNO"
						+ " ||' '||t.VAPEMATERNO ||','||t.VNOMBRES) AS descripcion,e.vdescripcion as estado, s.dfeccre as fecha"
						+ " from atd_doc_entr_seg s, trabajador t, tipo e where s.nderivado=t.nficha and s.vestado <> 'ESTA004' "
						+ "and t.vcodestado in ('EIRC01') and IND_EMAIL = 0 and s.vestado=e.vcodtipo and e.vobservaciones='ESTADO ATD' and s.ncorrelativo="
						+ nroCorrelativo + " and s.nano=" + nano + " and s.nareaderivado="
						+ areaDerivado + " order by ncodseg",
						new RowCallbackHandler() {
