...
public class TreeControllerMesa extends BaseBean implements Serializable {
	...
	public void inicializarArbol(String idDoc, String nanoLlave) {
		...
		rootNode = addNode(null,idDoc+":"+ area.getDescripcion(), new Area(idDoc, area.getDescripcion(), area.getOrigen()));
		model = new DefaultTreeModel(rootNode);
		selectedUserObject = (AreaUserObject) rootNode.getUserObject();
		selectedUserObject.setExpanded(true);
	...
	private DefaultMutableTreeNode addNode(DefaultMutableTreeNode parent,
		...
		// employee node
			...
			userObject.setText(area.getCodigo() + ", "
					+ area.getDescripcion()+ "- " + area.getEstado()+"- " +area.getFecha());
	...
	public void eventGenera(ActionEvent event) {
		...
		if (level == 0) {
			...
			for(Area p:docs)
			{
				addNode(node2, p.getDescripcion()+" Derivado Por :"+p.getOrigen(),new Area(p.getCodigo(), p.getDescripcion(), p.getOrigen()));
			}
			...
			for(Area p:areas)
			{
				addNode(node2, null	,new Area(p.getCodigo(), p.getDescripcion(), p.getEstado(), p.getFecha(), p.getIndicador()));
			}
		}else {
