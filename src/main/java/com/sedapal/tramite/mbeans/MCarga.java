...
public class MCarga implements Serializable {
	...
	public void uploadFile(ActionEvent event) {
		...
		if (extension.equals(".pdf") || (extension.equals(".PDF"))) {
			if (fileInfo.getStatus() == FileInfo.SAVED) {
				...
				indicador = (Integer) session.getAttribute("indicador");
				...
				try {
					...
					if (indicador == 1) {
						file.renameTo(
								new File(bundle.getString("path.entrada") + "/" + annio + "" + secuencial + ".pdf"));
					} else if (indicador == 2) {
						file.renameTo(
								new File(bundle.getString("path.salida") + "/" + annio + "" + secuencial + ".pdf"));
					} else if (indicador == 3) {
						file.renameTo(new File(
								bundle.getString("path.seguimiento") + "/" + annio + "" + secuencial + ".pdf"));
					} else if (indicador == 4) {
						file.renameTo(new File(bundle.getString("path.mesa") + "/" + annio + "" + secuencial + ".pdf"));
					} else if (indicador == 5) {
						file.renameTo(
								new File(bundle.getString("path.anexo") + "/" + annio + "" + secuencial + ".pdf"));
					}
					...
					if (session != null) {
						...
						if (indicador == 5) {
							session.setAttribute("anexoFile", secuencial + ".pdf");
						} else {
				...
				/*
				 * Eli comenta este codigo por la incidencia presentado 20/04/2017 else {
				 * System.out.println("Estoy evento tres"); mess = " Archivo corrupto."; }
				 */
	...
	public void cancelar(ActionEvent actionEvent) {
		...
		if (indicador == 5) {
			file = (File) session.getAttribute("anexoFile");
		} else {
	...
	public void ejecutarCarga() {
		...
		try {
			...
			this.msgAlerta = "Se leyeron " + contadorUsuInsert + " lineas del file";
			this.verAlerta = true;
	...
	/**
	 * <p>
	 * Allows a user to remove a file from a list of uploaded files. This methods
	 * assumes that a request param "fileName" has been set to a valid file name
	 * that the user wishes to remove or delete
	 * </p>
	 *
	...
	public MCarga() {
		/*
		 * try{ FacesContext fc = FacesContext.getCurrentInstance(); ExternalContext ec
		 * = fc.getExternalContext(); imgResource = new MyResource(ec,"logo.jpg");
	...
	public void inicia() {
		 ...
		 * fc.getExternalContext(); pdfResource = new
	...
	class MyResource implements Resource, Serializable {
		 ...
		 * array, and then serving the Resource from the ByteArrayInputStream, is not
		 * strictly necessary, but serves to illustrate that the Resource content need
		 * not come from an actual file, but can come from any source, and also be
		 * dynamically generated. In most cases, applications need not provide their own
		 * concrete implementations of Resource, but can instead simply make use of
		...
		public InputStream open() throws IOException {
			if (inputStream == null) {
				...
				String path = "/usr/local/apache2/htdocs/joomla/entrada/" + this.resourceName;
