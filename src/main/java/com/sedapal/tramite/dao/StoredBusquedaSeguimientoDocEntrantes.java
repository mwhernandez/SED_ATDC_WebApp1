...
public class StoredBusquedaSeguimientoDocEntrantes extends StoredProcedure implements RowMapper {

	private static final String SPROC_NAME = "ATDC_BUSQUEDA_SEG_DOC_ENT";	
	private static final String ANNO = "v_anno"; 
	private static final String AREA = "v_area"; 
	private static final String TIPODOC = "v_tipodoc";
	private static final String NUMDOC = "v_numdoc"; 
		
	
	
		
	public StoredBusquedaSeguimientoDocEntrantes() {
		// TODO Auto-generated constructor stub
	}

	public StoredBusquedaSeguimientoDocEntrantes(DataSource dataSource) {
		...
		declareParameter(new SqlParameter(NUMDOC, Types.VARCHAR));		
		declareParameter(new SqlOutParameter("consulta_seguimiento",OracleTypes.CURSOR, this));//cuenta_cursor?
		compile();
	}

	public Map execute(String anno, String area, String tipodoc, String numdoc) {
		System.out.println("Execute Stored..."+ anno+"   "+ area + " " +  tipodoc +" " +numdoc);
		Map inputs = new HashMap();			
		inputs.put(ANNO, anno);	
		inputs.put(AREA, area);
		inputs.put(TIPODOC, tipodoc);
		inputs.put(NUMDOC, numdoc);
	...
	public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		bean.setNseguimiento(rs.getInt("ncodseg"));
