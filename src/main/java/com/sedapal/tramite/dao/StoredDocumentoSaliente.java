...
public class StoredDocumentoSaliente extends StoredProcedure implements RowMapper {
	...
	private static final String SPROC_NAME = "S_GUIA.ATDC_DOCUMENTO_SALIENTE_V2";
	// private static final String NUM_TARJETA = "v_numero_tarjeta";
	private static final String NCODAREA = "v_codarea";
	private static final String FICHA_USUARIO = "v_ficha_usuario";
	...
	public StoredDocumentoSaliente(DataSource dataSource) {
		...
		declareParameter(new SqlParameter(FICHA_USUARIO, OracleTypes.NUMBER));
		declareParameter(new SqlOutParameter("documento", OracleTypes.CURSOR, this));// cuenta_cursor?
		compile();
	}

	public Map execute(String area, int fichaUsuario) {
		Map inputs = new HashMap();
		// inputs.put(NUM_TARJETA, "");
		inputs.put(NCODAREA, area);
		inputs.put(FICHA_USUARIO, fichaUsuario);
	...
	public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		bean.setNcasilla(rs.getInt("NCASILLA"));
		bean.setDfeccasilla(rs.getDate("FECCASILLA"));
		bean.setDhoracasilla(rs.getTime("FECCASILLA"));
		bean.setInd_remitente(rs.getInt("INDREMITENTE"));
		bean.setFichaVisador(rs.getInt("FVISADOR"));
