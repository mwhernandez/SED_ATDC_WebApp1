...
public class GenerarReporte {
    ...
    public  byte[] generarReporte() {
        byte[] result = null;
        logger.debug("[GenerarReporte.generarReporte]3) Inicio Generando");
    ...
    protected  JasperPrint generateJasperPrint() {
    	logger.debug("[GenerarReporte.generateJasperPrint]4)Inicio JasperPrint");
        ...
        try {
        	...
        	logger.debug("[GenerarReporte.generateJasperPrint]7)Cargando Objeto");
            report = (JasperReport) JRLoader.loadObject(xmlDesign);
    ...
    //        parameters.put("P_TITULO", "BCP");
            logger.debug("[GenerarReporte.generateJasperPrint]9)Obteniendo Conexion jdbc.");
            connection= dataSource.getConnection();         
            logger.debug("[GenerarReporte.generateJasperPrint]10)Conexion ok.");
            if(seguimientoDoc==null)
             result = JasperFillManager.fillReport(report, parametros, connection);
            else
            	result = JasperFillManager.fillReport(report, parametros, new JRBeanCollectionDataSource(seguimientoDoc));
            

        } catch (Exception ex) {
        	logger.error("[GenerarReporte.generateJasperPrint]",ex);
            ex.printStackTrace();
        }
        finally{
        	if(connection!=null)
    ...
    public  InputStream getXMLDesign() {
        ...
        logger.debug("[GenerarReporte.getXMLDesign] 5) Obteniendo Recurso Jasper");
        try {
            result = GenerarReporte.class.getResourceAsStream(this.nombreReporteJasper);
            logger.debug("[GenerarReporte.getXMLDesign] 6) Recurso Obtenido:"+ result);
            ...
            logger.error("[GenerarReporte.getXMLDesign]",ex);
            ex.printStackTrace();
        }
        return result;
    }
